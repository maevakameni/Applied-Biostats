...
#Since the union-shop is already described as dummy no need to convert its type.
```
...
```
Now we will analysis the distribution of the differents variable factors according to our groups namely Right-to-work states and Union-shop States
...
pairs(data[, 2:6], labels = c("Degree of conflict","Degree of unionization","Union Shop","Sector","Ag force"), main = "Correlation plots associated with the numerical variables", cex.labels = 1.1, upper.panel = panel.cor)
...
--> multiple linear regression (2 digits and ^ on estimates)
